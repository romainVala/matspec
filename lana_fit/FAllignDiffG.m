...
function [AmListIniD,resnorm,output] = FAllignDiffG(dcy,sumfidMD1,AmListIni,metname);
...
%lowb(1,:)=0.5*AmListIni(1,1);

lowb(2,:)=AmListIni(2,1)*0.8 ;


lowb(3,:)=-5; %freq

lowb(4,:)=AmListIni(4,1)-0.02;
lowb(5,:)=0.9;

%Setting low bound limit
upperb=AmListIni;
upperb(1,:)=6*AmListIni(1,1);
upperb(2,:)=3*AmListIni(2,1);
...
upperb(3,:)=4;
upperb(4,:)=AmListIni(4,1)+0.02;
upperb(5,:)=2;


options=optimset('Display','final','MaxFunEvals',1000,'MaxIter',1000,'TolX',10^-18,'TolFun',10^-18);
...
tic
[AmListIniD,resnorm,residual,exitflag,output]=lsqcurvefit(@FitIniDG,AmListIni,timeD,[real(fftExp(XpointsD(1):XpointsD(2))) imag(fftExp(XpointsD(1):XpointsD(2)))],lowb,upperb,options,sumfidMD1);
