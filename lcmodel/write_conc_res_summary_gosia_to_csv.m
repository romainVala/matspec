function write_conc_res_summary_gosia_to_csv(conc,resname,field_list)
...
for kf = 1:length(field_list)
  fprintf(fid,',%s,%s,%s',field_list{kf},field_list{kf},field_list{kf});
end
...
for kf = 1:length(field_list)
  fprintf(fid,',mean,%s,CRLB',stdstr);
end
...
for npool = 1:length(conc)
  ...
  for kf = 1:length(field_list)
    ...
    sdaa =  ['SD',metab];
    if isfield(conc(npool),sdaa)
      aa = getfield(conc(npool),sdaa);
      aa(isnan(aa))=[];
