...
%   Example 1
%   ---------
%   Use ANALYZE75INFO to read the header file CT_HAND.hdr. This file
...
%
%       metadata = analyze75info('CT_HAND', 'ByteOrder', 'ieee-be');
%
...
%                         and volumes are of the same size. A value 'y'
%                         indicates that the data is regular while 'n'
...
%   $Revision: 1.1.6.4 $  $Date: 2006/06/15 20:10:28 $
...
fclose(fid);
    ...
    if ~((headerSize >= extendedRange(1)) && ...
        ...
        if (~secondAttempt)
            ...
            fclose(fid);
            eid = 'Images:analyze75info:hdrFileOpen';
            msg = ['Failed validating header size. File may not be '...
                   'a valid Analyze 7.5 format file.'];
            error(eid, msg);
        end

        % Generate warning if incorrect ByteOrder was provided by user.
        if userSupplied.ByteOrder
            msgid = 'Images:analyze75info:incorrectByteOrder';
            msg = ['Incorrect ByteOrder "%s", Using "%s" ' ...
                   'format to read the file.'];
            warning(msgid , msg , oldByteOrder, args.ByteOrder);
        end  % if
        ...
        % Metadata struct.
        [pname fname ext] = fileparts(metadata.Filename);
        ...
        try
            ...
            msgid = 'Images:analyze75info:imgFileSize';
            msg = ['Unable to obtain ImgFileSize.' ...
                   ' File "%s" may not exist.'];
            warning(msgid,msg,ImgFilename);
        end  %  try-catch
    ...
    if isempty(temp)
        if ~alreadyWarned
            msgid = 'Images:analyze75info:truncatedHeaderFile';
            msgstr = ['End of file found before the end of the header ' ...
                ' was reached.\nFile may be an invalid Analyze 7.5 file' ...
                ' or corrupt.\nOutput structure may not contain' ...
                ' complete file information.'];
            warning(msgid, msgstr);
...
iptcheckinput(filename, {'char'}, {}, mfilename, 'FileName', 1);

% Verify that filename is a valid Analyze 7.5 format file.
if ~isanalyze75(filename)
    eid = 'Images:analyze75info:invalidAnalyze75file';
    msg = '%s is not a valid Analyze 7.5 format file.';
...
if (nargin > 1)
    ...
    for k = 1:2:length(varargin)
        ...
        iptcheckinput(param ,{'char'},{},mfilename,'Parameter Name', k+1);

        % Compare and validate Parameter Name.
        idx = strmatch(param, paramStrings);
        if (isempty(idx))
            eid = 'Images:analyze75info:unrecognizedParameterName';
            msg = 'Unrecognized parameter name "%s".';
            error(eid, msg, param);
        elseif (length(idx) > 1)
            eid = 'Images:analyze75info:ambiguousParameterName';
            msg = 'Ambiguous parameter name "%s".';
            error(eid, msg, param);
        end  % if
        ...
        switch (paramStrings{idx})
            case 'byteorder'
                if (k == length(varargin))
                    eid = 'Images:analyze75info:missingByteOrder';
                    msg = 'No value specified for parameter "ByteOrder".';
                    error(eid, msg);
                else
                    paramValues = {'ieee-le','ieee-be'};
                    value = varargin{k+1};
                ...
                if (isempty(idx))
                    eid = 'Images:analyze75info:unrecognizedParameterValue';
                    msg = ['Unrecognized parameter value "%s", Valid ' ...
                        'values for "ByteOrder" are "%s" and "%s".'];
                    error(eid, msg, value,paramValues{:});
                elseif (length(idx) > 1)
                    eid = 'Images:analyze75info:ambiguousParameterValue';
                    msg = ['Unrecognized parameter value "%s", Valid ' ...
                        'values for "ByteOrder" are "%s" and "%s".'];
                    error(eid, msg, value,paramValues{:});
                end  % if
