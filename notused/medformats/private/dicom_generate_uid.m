...
%   $Revision: 1.1.6.2 $  $Date: 2006/05/24 03:33:25 $
...
case {'instance', 'series', 'study'}
    ...
    elseif (ispc)
        ...
        % Create a GUID value and truncate it to fit in DICOM's length
        % requirements (choose a value from the middle).
...
if (~isempty(uid_root))
    uid_part = remove_leading_zeros(uid_root);
    return
end
...
if (isempty(machine_part))
    ...
    if (isunix)
        ...
        [mach, status] = perl('private/dicom_get_machine_parts.pl', ...
                              lower(computer));

        if (status ~= 0)
            % Print error from Perl script, minus line feeds.
            error('Images:dicom_generate_uid:perlError', mach(1:(end - 1)))
        end
        ...
        error('Images:dicom_generate_uid:instanceIsUnixOnly', ...
              'UID instance part is supported for UNIX platforms only.')
...
uid_part = remove_leading_zeros(machine_part);
...
% Put it all together.
out = remove_leading_zeros([id_part process_part]);
