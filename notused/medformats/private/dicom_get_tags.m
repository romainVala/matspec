...
%   $Revision: 1.1.6.2 $  $Date: 2005/11/15 01:04:28 $
...
% The file should be at the beginning of the file metadata.
...
while (more_items)
    ...
    if ((isequal(file.Current_VR, 'Explicit')) && (tag(1) ~= 65534))
        ...
        vr = fread(file.FID, 2, 'char=>char', file.Current_Endian)';
    ...
    if (tag(2) == 16)  % (0002,0010)
        ...
        data = fread(file.FID, len, 'char=>char', file.Current_Endian)';
        info.TransferSyntaxUID = my_deblank(data);
        
    else
        
        if (~isequal(len, dicom_undefined_length()))
