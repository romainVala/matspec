...
%   $Revision: 1.1.6.1 $  $Date: 2005/05/27 14:09:04 $
...
for p = flist
    ...
    while (more_fragments) 
        ...
        % Handle badly formed DICOM files.  This part is overly
        % engineered to allow a particular set of DICOM files to
        % pass.  A more elegant solution is used in dicomparse.mex,
        % which will eventually replace this codebase.
        if (~isequal(attr.Group, 65534))
          
          dicom_close_msg(file);
          error('Images:dicomparse:badEncapsulatedData', ...
                'Corrupt encapsulated image data.')
          
        elseif (rem(attr.Length, 2) ~= 0)
          
          % Only let a certain number of odd-length attributes slide.
          if (~safe_mode)

            txt = sprintf('Attribute (%04X,%04X) has an illegal length (%ld).\nSwitching to a safe reading mode.  This operation may fail.', ...
                          attr.Group, attr.Element, attr.Length);
            warning('Images:dicomparse:oddLength', txt)
          ...
          if (feof(file.FID))
            ...
            dicom_close_msg(file);
            error('Images:dicomparse:badEncapsulatedData', ...
                  'Attempt to read past end of file.  Stopping.')
          ...
          if (safety_violations > 10)
            ...
            dicom_close_msg(file);
            error('Images:dicomparse:badEncapsulatedData', ...
                  'Too many odd length attributes read.  Stopping.')
        ...
        try
          ...
          m = lasterror;
          if (isequal(m.identifier, 'MATLAB:nomem'))
...
for p = 1:total_fragments
    ...
    %
    % DICOM_TYPECAST currently only swaps if the output is UINT8/INT8.
    % Swap the bytes before calling DICOM_TYPECAST.
    if (swap)
        tmp = offset_tables{p};
    ...
    offset_tables{p} = double(dicom_typecast(offset_tables{p}, 'uint32'));
...
if (info.BitsAllocated == 8)
    ...
    if (swap)
        ...
        pixel_cells = dicom_typecast(pixel_cells, 'uint8', 1);
        pixel_cells = dicom_typecast(pixel_cells, pixcell_type);
...
for p = 1:(nSamples)
    ...
    sample = dicom_typecast(sample, container_type);
...
if (info.PixelRepresentation == 1)
    ...
    case 'uint8'
        pixel_cells = dicom_typecast(pixel_cells, 'int8');
    case 'uint16'
        pixel_cells = dicom_typecast(pixel_cells, 'int16');
    case 'uint32'
        pixel_cells = dicom_typecast(pixel_cells, 'int32');
    case 'uint64'
        pixel_cells = dicom_typecast(pixel_cells, 'int64');
    end
...
if (pixelrep)
    ...
    case 'uint8'
        ...
        out = dicom_typecast(in(:), 'int8');
        out = reshape(out, size(in));
        
    case 'uint16'
    
        out = dicom_typecast(in(:), 'int16');
        out = reshape(out, size(in));
