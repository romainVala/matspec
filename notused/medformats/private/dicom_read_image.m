...
%   $Revision: 1.1.6.1 $  $Date: 2005/05/27 14:09:06 $
...
if ((isfield(info, 'PhotometricInterpretation')) && ...
    ...
    switch (class(X))
    case 'uint8'
        alpha = repmat(uint8(0), info.Rows, info.Columns, 1, size(X, 4));
        
    case 'uint16'
        alpha = repmat(uint16(0), info.Rows, info.Columns, 1, size(X, 4));
        
    case 'uint32'
        alpha = repmat(uint32(0), info.Rows, info.Columns, 1, size(X, 4));
...
if ((isfield(info, 'PhotometricInterpretation')) && ...
    ...
    red = double(info.RedPaletteColorLookupTableData) ./ ...
          (2 ^ double(info.RedPaletteColorLookupTableDescriptor(3)) - 1);
    
    green = double(info.GreenPaletteColorLookupTableData) ./ ...
          (2 ^ double(info.GreenPaletteColorLookupTableDescriptor(3)) - 1);
    
    blue = double(info.BluePaletteColorLookupTableData) ./ ...
          (2 ^ double(info.BluePaletteColorLookupTableDescriptor(3)) - 1);
