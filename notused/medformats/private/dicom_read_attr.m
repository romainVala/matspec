...
%   $Revision: 1.1.6.3 $  $Date: 2006/06/15 20:10:41 $
...
if ((~isequal(attr.VR, 'SQ')) && ...
    (~isequal(attr.Length, dicom_undefined_length())))
...
elseif ((rem(attr.Group, 2) == 1) && ...
    ...
    % in private attributes.  The VR can't be reliably determined
    % from the data dictionary, since they're private.
...
try
    ...
    msg = lasterr;
    error('Images:dicom_read_attr:attrDetails', '%s', msg)
...
if ((attr.Length == dicom_undefined_length) || ...
    (isequal(attr.VR, 'SQ')) || ...
    (attr.Group == 65534))
...
elseif (findstr(attr.Name, 'xx_Creator'))
    ...
    if (~isempty(findstr('OverlayData', attr.Name)))
        ...
        ol_width = double(info.(['OverlayColumns_' ol_num]));
        ol_height = double(info.(['OverlayRows_' ol_num]));
        
        if (isfield(info, ['NumberOfFramesInOverlay_' ol_num]))
            ol_frames = double(info.(['NumberOfFramesInOverlay_' ol_num]));
        else
            ol_frames = 1;
        end
...
if (isempty(attr_dict))
    if ((rem(attr.Group, 2) == 1) && (attr.Group < 65534))  % 0xFFFE == 65534.
        ...
        attr.Name = dicomlookup_actions(attr.Group, attr.Element);
