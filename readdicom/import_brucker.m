function fid = import_brucker(fi)
...
%ff = get_subdir_regex_files(p,'fid.raw$');

for kk=1:length(ff)
  fid(kk) = import_brucker(ff{kk});
end
...
tic
pt_file=fopen(fullfile(PathName,FileName),'r');
...
while flag_method==0 ;
    ...
    if strmatch('##$PVM_EchoTime=',line);
        MethTE=(strtok(line,'##$Method='));
    end
    if strmatch('##$PVM_RepetitionTime=',line);
        MethTR=(strtok(line,'##$Method='));
    end
...
if pt_refscan
    ...
    %fid.water_ref = eddycorrection(fid.water_ref,fid.water_ref);
end
toc
