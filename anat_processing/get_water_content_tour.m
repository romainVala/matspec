...
for npol = 1:length(cmet)
  ...
  for ns = 1:length(suj)
    sujname = suj{ns};
    if findstr(sujname,'_sujet_')
      ii=findstr(sujname,'_');
      sujreg = sujname(1:ii(3)-1);
      serreg = sujname(ii(4)+1:end);      
    else
      ii=findstr(sujname,'_');
      sujreg = sujname(1:ii(2)-1);
      serreg = sujname(ii(3)+1:end);
    end


    aref = get_subdir_regex(anat_dir,sujreg,'S176$');
    c1 =  get_subdir_regex_files(aref,'^c1.*img',1);
    c2 =  get_subdir_regex_files(aref,'^c2.*img',1);
    c3 =  get_subdir_regex_files(aref,'^c3.*img',1);
