...
%rawdir = '/home/romain/data/spectro/tourette/lcmodel/mega_first';
rawdir = '/home/romain/data/spectro/tourette/lcmodel/mega_inv';
...
region_wc = {'MCL32$','STR_32$','Thal_32$'};
...
for nbg= 1:2
  ...
  for nbr=1:length(region_wc)
    ...
    fc(1).phase_cor = 0;
    fc(1).freq_cor = 0;
    fc(1).max_freq_shift = 0;
    
    for nbsuj=1:length(f)
      
%      if~isempty(findstr( f(nbsuj).SubjectID,'SL_Tourettespectro_30t_E1')) & ~isempty(findstr(f(nbsuj).ser_dir,'S28_MEGA_PRESS_Thal_32'))
      if~isempty(findstr( f(nbsuj).SubjectID,'Tourette_sujet_127p')) & ~isempty(findstr(f(nbsuj).ser_dir,'Thal_MEGA'))
	par.ref_metab='CRE';
      else
    ...
    try
      fc(nbsuj) = processing_MEGA(f(nbsuj),par);
    catch
...
%    plot_spectrum(fc)
    ...
    ff = remove_field_string(ff,'SerDescr','MEGA-PRESS ');
    aa=length(ff(1).SerDescr);
    
    if nbr==3
      ff = remove_field_string(ff,'SerDescr',[5:aa]);
    else
      ff = remove_field_string(ff,'SerDescr',[4:aa]);
    end
    
    group_name{1} = [ ff(1).SerDescr,'_',group{nbg}]
    
    %plot_spectrum(ff)
    %keyboard
    
    fall(nb_sub_group).group  =  average_fid_mega(ff);
    ...
    for nsuj=1:length(fall(nb_sub_group).group)
      ...
      wref = processing_MEGA(wref,parw);
...
write_conc_res_summary_stat(ccs,'totooS.csv')

write_conc_res_to_csv(ccs,'totoo.csv')
...
write_conc_res_summary_stat(ccpre,'totopS.csv')
